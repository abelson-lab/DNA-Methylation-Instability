...
gse105018_normal_blood_counts = pd.read_csv('data/GSE105018_normal_blood_counts.csv', header=None).iloc[:,0]
...
unknown = np.mean(pr.read_r('data/GSE124413/healthyBM/beta_values.RDS')[None], axis=1)
chinese = np.mean(pr.read_r('data/GSE141682_RAW/beta_values.RDS')[None], axis=1)
...
for group in stable.keys():
    for num in [0,1]: # unmethylated, methylated
        ...
        bplot = ax.boxplot(mean_bvals, patch_artist=True,
                        medianprops=dict(linewidth=1, color='black'), sym='.')
        for patch in bplot['boxes']: patch.set_facecolor('red')
        ax.set_title(f"{group}: {['UM','M'][num]} (n={len(sites)})")
        ax.set_xticklabels(['B cell', 'CD4 T cell', 'CD8 T cell', 'Granulocyte', 'Monocyte'])
...
# with open('data/stable_sites.pkl', 'wb') as fout:
#     pickle.dump(stable, fout, pickle.HIGHEST_PROTOCOL)

# stable['ALL'][0].to_csv('data/ESL_U.csv', header=None, index=False)
