...
# %%
### Load 450K manifest
manifest = pd.read_csv('data/general/humanmethylation450_15017482_v1-2.csv',
...
valid_probes = pd.read_csv('data/general/valid_probes.csv', header=None).iloc[:,0]
manifest = manifest[manifest['IlmnID'].isin(valid_probes)]
manifest.set_index('IlmnID', inplace=True)

# %%
### Load stable sites
with open('data/general/stable_sites.pkl', 'rb') as fin:
...
with open('data/general/pan-cancer-3019_beta_binarized.pkl', 'rb') as fin:
